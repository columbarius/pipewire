			     ...
			     struct spa_pod_builder *builder)
{
	...
	if (this->port[port_id].have_format
		...
		if (!find_EnumFormatInfo(&this->state.base, index-1, spa_vulkan_blit_get_buffer_caps(&this->state, direction), &fmt_index, &has_modifier))
			return 0;
	} else {
		if (!find_EnumFormatInfo(&this->state.base, index, spa_vulkan_blit_get_buffer_caps(&this->state, direction), &fmt_index, &has_modifier))
			return 0;
	}

	const struct vulkan_format_info *f_info = &this->state.base.formatInfos[fmt_index];
	spa_log_info(this->log, NAME": enum_formats idx: %d, format %d, has_modifier %d", index, f_info->spa_format, has_modifier);
...
static int clear_buffers(struct impl *this, struct port *port)
{
	if (port->n_buffers > 0) {
		...
		spa_vulkan_blit_stop(&this->state);
		spa_vulkan_blit_use_buffers(&this->state, &this->state.streams[port->stream_id], 0, &port->current_format.info.dsp, 0, NULL);
		port->n_buffers = 0;
			   ...
			   const struct spa_pod *format)
{
	...
	if (format == NULL) {
		...
		this->state.dim.width = this->position->video.size.width;
		this->state.dim.height = this->position->video.size.height;
		...
		if (port->direction == SPA_DIRECTION_OUTPUT
			...
			if (spa_vulkan_blit_fixate_modifier(&this->state, &this->state.streams[port->stream_id], &info.info.dsp, modifierCount, modifiers, &fixed_modifier) != 0)
				return -EINVAL;
			   ...
			   uint32_t n_buffers)
{
	...
	spa_vulkan_blit_use_buffers(&this->state, &this->state.streams[port->stream_id], flags, &port->current_format.info.dsp, n_buffers, buffers);
